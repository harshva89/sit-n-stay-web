...
<html>
    <head>
        ...
        <title>Sit & Stay - Analytics</title>
...
<h1>Orders placed during various months:</h1>
    ...
    <br><br>
    <h1>Newsletter Subscriber Details</h1>
    <?php  
    $sql = "select count(*) from subscribers;";
    $result = mysqli_query($con,$sql);
    while($row = mysqli_fetch_array($result)){
       echo "Total number of subscribers: ".$row[0];
    break;
        }
    ?>
    <br><br>
    <form action="subscribersList.php">
    <input type="submit" value="View all" class="button_1"></form>
    <br><br>
    <h1>Queries Received by users</h1>
    <?php  
    $sql = "select count(*) from contactus;";
    $result = mysqli_query($con,$sql);
    while($row = mysqli_fetch_array($result)){
       echo "Total number of queries recieved: ".$row[0];
    break;
        }
    ?>
    <br><br>
